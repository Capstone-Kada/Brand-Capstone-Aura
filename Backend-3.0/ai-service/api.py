...
# Batas ukuran file upload (5 MB) -- mencegah foto beresolusi sangat besar
# membebani proses resize/preprocessing secara tidak wajar.
...
async def analyze_face(file: UploadFile = File(...)):
    ...
    # `analyze_static_image()` di ML menerima PATH file, bukan bytes di memori
    # -- karena itu ditulis dulu ke file sementara. `delete=False` + `os.unlink`
    # manual di finally dipakai (bukan `with tempfile...` biasa) karena di
    # Windows, file yang masih terbuka oleh proses lain (cv2.imread) tidak
    # bisa langsung dihapus sebelum handle-nya ditutup penuh.
    ...
    try:
        ...
        # Kegagalan tak terduga di luar jalur "wajah tidak terdeteksi" yang
        # sudah ditangani ML (mis. file korup, format tak terbaca cv2.imread).
        raise HTTPException(
            status_code=422,
            detail=f"Gagal memproses gambar: {processing_error}",
