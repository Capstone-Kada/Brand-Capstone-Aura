...
async function fetchPage(skip: number, limit: number): Promise<SocoProduct[]> {
  ...
  const response = await axios.get<{ success: boolean; data: SocoProduct[] }>(CATALOG_API, {
    ...
    headers: {
      ...
      'User-Agent': 'AuraAI-CapstoneBot/1.0 (+educational; makeup-catalog-seeder)',
    },
    timeout: 30_000,
  });
...
async function upsertProduct(
  ...
  const undertoneMatch =
    nameLower.includes('warm') || nameLower.includes('peach') || nameLower.includes('coral')
      ? 'warm'
      : nameLower.includes('cool') || nameLower.includes('rose') || nameLower.includes('berry')
        ? 'cool'
        : 'universal';
