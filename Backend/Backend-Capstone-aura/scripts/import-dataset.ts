...
async function importProducts(rows: ProductRow[]): Promise<{ created: number; updated: number }> {
...
async function importRecommendationRules(rows: RecommendationRow[]): Promise<{ created: number; skipped: number }> {
...
async function importShadeMappings(rows: ShadeMappingRow[]): Promise<number> {
  for (const row of rows) {
...
async function backfillProductToneCoverage(): Promise<number> {
...
async function resetDatasetData(): Promise<void> {
...
async function main(): Promise<void> {
  ...
  const productResult = await importProducts(productRows);
  // eslint-disable-next-line no-console
  console.log(`Products: created=${productResult.created} updated=${productResult.updated}`);

  const ruleResult = await importRecommendationRules(recommendationRows);
  // eslint-disable-next-line no-console
  console.log(`Recommendation rules: upserted=${ruleResult.created} skipped=${ruleResult.skipped}`);

  const shadeMappingCount = await importShadeMappings(shadeMappingRows);
  // eslint-disable-next-line no-console
  console.log(`Shade mappings: upserted=${shadeMappingCount}`);

  const backfillCount = await backfillProductToneCoverage();
  // eslint-disable-next-line no-console
